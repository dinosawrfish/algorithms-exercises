...
class LinkedList {
  ...
  delete(index) {
    ...
    const nodeToExcise = node.next;

    if (!nodeToExcise) return null;
    node.next = nodeToExcise.next;
    if(!node.next) this.tail = node.next; //wouldn't this be node

    this.length--;
    return nodeToExcise.value;
  }
}
